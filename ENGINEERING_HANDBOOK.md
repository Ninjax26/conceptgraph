...
- Tests cover thirty-eight focused rules and mocked service cases. There are still no real API, worker, database, S3, frontend, load, penetration, or end-to-end integration tests in the repository.
- Qdrant cluster migrations require `python -m scripts.reconcile_ready_vectors --apply`; it invalidates false READY records with zero vectors without deleting source PDFs or fabricating readiness.
